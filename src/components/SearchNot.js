...
export const SearchNot = ({SetMyNotes}) => {
    ...
    if(Value===""){
      ...
      const lowercaseValue = Value.toLowerCase();
       const foundNotes = MyOldNotes.filter(
          (item) =>
          item.NotTitle.toLowerCase().startsWith(lowercaseValue) ||
          item.NotBody.toLowerCase().startsWith(lowercaseValue)
        );
        SetMyNotes(foundNotes);
  }
  ...
  return (
    <View style={styles.main}>
        <TextInput value={Value} onChangeText={(text)=>setValue(text)} placeholder='Search in notes...' style={styles.TextInputSeach}/>
